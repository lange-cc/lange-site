...
function asyncFunction (data, cb) {
    setTimeout(() => {
        ...
        var html2 = '<div class="col-lg-3" id="img_id_'+data.id+'"><div class="uploaded-img-widget">'+
        '<div id="bg_id_'+data.id+'" class="bg-img" style="background: #0F1215 url('+file_location+'all-images/thumbnail/'+data.img_name+') no-repeat center;background-size: cover;min-height: 180px;position: relative;">'+
        '<i id="loader_id_'+data.id+'" class="loader fa fa-spinner fa-spin"></i>'+
        '</div><div class="upload-option"><a href="" class="btn select-file" data-type="logo" data-url="'+data.img_name+'">Select</a>'+
        '<a href="" class="btn delete-img pull-right" data-id="'+data.id+'" data-name="'+data.img_name+'">Delete</a></div></div></div>';
        $('#menu2').append(html2);
